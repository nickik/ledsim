...
const UDP_PORT: u32 = 24;

const RGB_SIZE: usize = 3;
const BUFFER_SIZE: usize = HEIGHT * WIDTH * RGB_SIZE;

fn main() -> std::io::Result<()> {
    // Bind the UDP socket to a specific address and port
    let socket = UdpSocket::bind("127.0.0.1:54321")?;
    println!("UDP server listening on 127.0.0.1:54321");
    ...
    loop {
        ...
        let mut flat_array: [(u8, u8, u8); HEIGHT * WIDTH] = [(0, 0, 0); HEIGHT * WIDTH];
...
*/
    ...
    loop {
        ...
        // Convert the received data into a 2D array of RGB tuples
        let mut array: Vec<Vec<(u8, u8, u8)>> = Vec::new();

        for row in 0..HEIGHT {
            let mut current_row = Vec::new();
            for col in 0..WIDTH {
                let start_index = (row * WIDTH + col) * RGB_SIZE;
                let red = buffer[start_index];
                let green = buffer[start_index + 1];
                let blue = buffer[start_index + 2];

                current_row.push((red, green, blue));
            }
            array.push(current_row);
        }

        println!("Converted data into a 2D array of RGB colors:");

        // Display the first few rows for confirmation
        for (i, row) in array.iter().enumerate().take(5) {
            println!("Row {}: {:?}", i, row);
        }

        // You can now use the `array` as needed...
    }
